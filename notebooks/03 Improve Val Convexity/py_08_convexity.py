# -*- coding: utf-8 -*-
"""
Guerrero (2007) penalized trend on S&P 500 with golden-section tuning of s,
...
Pipeline:
  ...
  3) For each differencing order d in d_list:
       - Minimize validation MSE J_d(s) over s∈(0,1) via golden-section search.
       - Print MSE at each iteration (sanity check).
...
# ============================================================
#  Global cache for eigenvalues of K K' (by (N, d))
# ============================================================
...
# ============================================================
#  Core numerical routines (Guerrero penalized trend)
# ============================================================
...
def lambda_from_s_unit(
    ...
    For d >= 1:
        tr[(I + λ K'K)^(-1)] = d + sum_{i=1}^{N-d} 1 / (1 + λ ν_i),
    ...
    Eigenvalues ν are cached per (N, d) via _get_nu_for_lambda.
    """
    s_unit = float(s_unit)
    if s_unit <= 0.0:
        return 0.0
    ...
    if d == 0:
        # closed form when there's no differencing penalty
        return s_unit / (1.0 - s_unit)

    # eigenvalues of K K' from cache
    nu = _get_nu_for_lambda(N, d, K)
    ...
    lo, hi = 0.0, 1.0
    # bracket so that S_raw(hi) >= target (or hi becomes very large)
...
def fit_penalized_trend_dense(
    ...
    Returns:
        ...
        used_gpu    : bool (always False here)
    """
...
# ============================================================
#  Validation loss J_d(s) = MSE on validation for fixed d, s
# ============================================================
...
def val_loss_for_d_s(
    ...
    Protocol:
      - Fit Guerrero trend on TRAIN only with (d, s).
      - Forecast over VAL horizon.
      - Compute MSE between forecast and Z_val.
    """
    ...
    try:
        t_hat, m_hat, lam, sigma2_hat, Ainv, s_real, used_gpu = fit_penalized_trend_dense(
            Z_train, d=d, s_unit=s_unit
        )
    except Exception:
        # treat blown-up configurations as terrible
        return np.inf
...
# ============================================================
#  Golden-section search for s (per d)
# ============================================================
...
def minimize_s_golden(
    ...
    """
    Minimize validation MSE J_d(s) over s ∈ [s_min, s_max] for fixed d
    using golden-section search.

    Uses val_loss_for_d_s(Z_train, Z_val, d, s).
    Prints MSE at each iteration if verbose=True.
    """
    Z_train = np.asarray(Z_train, dtype=float).ravel()
    Z_val = np.asarray(Z_val, dtype=float).ravel()
    ...
    f_c = val_loss_for_d_s(Z_train, Z_val, d, c)
    f_d = val_loss_for_d_s(Z_train, Z_val, d, d_s)

    if verbose:
        print(f"[d={d}] init: a={a:.4f}, c={c:.4f}, d={d_s:.4f}, b={b:.4f}")
        print(f"[d={d}]      f(c)={f_c:.6e}, f(d)={f_d:.6e}")
    ...
    for k in range(n_iter):
        ...
        if verbose:
            print(
                ...
                f"f(c)={f_c:.6e}, f(d)={f_d:.6e}"
            )

    # best s is midpoint of final [a,b]
    s_opt = 0.5 * (a + b)
    loss_opt = val_loss_for_d_s(Z_train, Z_val, d, s_opt)
    if verbose:
...
def main():
    ...
    # 3. For each d, minimize validation MSE over s via golden-section
    d_list = [1, 2, 3]  # you can try [1,2,3,4] if numerically stable
    best_by_d: Dict[int, Dict[str, Any]] = {}

    for d in d_list:
        print(f"\n=== Minimizing validation MSE in s for d={d} (golden-section) ===")
